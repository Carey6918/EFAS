...
public interface CorpPertainsDao extends JpaRepository<CorpPertainsPO, CorpKey> {
    ...
    @Query(value = "select s from CorpPertainsPO s, TMCorpCorpPertainsPO m where m.org=?1 and m.id=?2 and m.seqId=?3 and s.corpKey.org=m.subOrg and s.corpKey.id = m.subId and s.corpKey.seqId=m.subSeqId")
